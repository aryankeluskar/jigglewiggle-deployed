...
// Score-specific color schemes for particles and effects
const SCORE_COLORS: Record<ScoreType, { primary: string; secondary: string; glow: string }> = {
  perfect: { primary: "rgba(57, 255, 20, 1)", secondary: "rgba(0, 255, 170, 1)", glow: "rgba(57, 255, 20, 0.8)" },
  great: { primary: "rgba(255, 225, 0, 1)", secondary: "rgba(255, 170, 0, 1)", glow: "rgba(255, 225, 0, 0.8)" },
  ok: { primary: "rgba(0, 180, 255, 1)", secondary: "rgba(0, 255, 255, 1)", glow: "rgba(0, 180, 255, 0.6)" },
  almost: { primary: "rgba(255, 100, 0, 1)", secondary: "rgba(255, 50, 50, 1)", glow: "rgba(255, 100, 0, 0.6)" },
  miss: { primary: "rgba(255, 0, 60, 1)", secondary: "rgba(150, 0, 60, 1)", glow: "rgba(255, 0, 60, 0.5)" },
};
...
export default function ScorePopup({ score, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    if (score && !isShowingRef.current && score !== lastShownScoreRef.current) {
      ...
      // Hide after animation completes - longer for more impact
  ...
  const particleCount = isHighScore ? 30 : 20;

  return (
    <>
      {/* Screen shake container for high scores */}
      ...
      >
        ...
        {/* Main score image with enhanced effects */}
        <div
          className={`score-popup ${isVisible ? "score-popup-enter" : ""}`}
          style={{
            transform: `rotate(${rotation}deg) scale(${isVisible ? 1 : 0.3})`,
          }}
        >
